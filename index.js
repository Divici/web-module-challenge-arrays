...
function getAverageWordLength(array){
  ...
  for (let i=0;i<array.length;i++){
    let wordCount=1;
    if(array[i].includes(' ')){
      wordCount+= (array[i].split(' ').length-1);
    }
    wordTotal+=wordCount;
  }
...
function getRandomFlavors(array1, array2, array3, array4){
  /*code here*/
  const randomPool = array1.concat(array2, array3, array4);
  let randomFlavors=[];

  for (let i =0;i<31;i++){
    randomFlavors.push(randomPool[Math.floor(Math.random()*randomPool.length)]);
  }
